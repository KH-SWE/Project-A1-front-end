...
// Keep the base classes separate from the default width so certain inputs can opt out
const COMMON_INPUT_CLASS =
	"bg-white px-6 py-5 rounded-3xl font-inter-regular mb-3";
const DEFAULT_WIDTH_CLASS = "w-11/12";
...
const InputField: React.FC<any> = ({ className, style, noWidth, ...props }) => {
	// `noWidth` allows inline/side-by-side layouts to control width via container styles
	const widthClass = noWidth ? "" : DEFAULT_WIDTH_CLASS;
	const mergedClass = [COMMON_INPUT_CLASS, widthClass, className]
		.filter(Boolean)
		.join(" ");
	...
	return (
		<TextInput
			...
			style={[styles.inputShadow, style]}
		/>
	);
};
										...
										if (
											...
											if (selected.id != null && opt.id != null) {
												...
												const selName =
													...
													selected.name;
												const optName =
													opt.major_name ?? opt.faculty_name ?? opt.name;
											...
											if (selected === opt) isSelected = true;
												...
												isSelected =
													selected === opt.enumlabel ||
													selected === opt.major_name ||
													selected === opt.faculty_name ||
													selected === opt.name;
											}
										}
									}
...
export default function SignupScreen() {
		...
		try {
			...
			UIManager.measureLayout(
				...
				(_left: number, top: number, _width: number, _height: number) => {
					...
					setTimeout(
						() => scrollRef.current?.scrollTo({ y: desired, animated: true }),
						50
					);
				}
			);
	...
	const handleRegister = async () => {
		// client-side checks
		if (!firstName.trim()) return Alert.alert("Please enter your first name");
		if (!lastName.trim()) return Alert.alert("Please enter your last name");
							...
							>
								{step === 0 && (
									<>
										...
										<View className="w-11/12" style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
								...
								{step === 1 && (
									<>
										...
										{majors.length === 0 ? (
											...
											<Dropdown
												...
												onSelect={(item: any) => {
													setMajorId(item.id);
													setFacultyId(
														item.faculty_id ?? item.facultyId ?? null
													);
												}}
										...
										{faculties.length === 0 ? (
											...
											<Dropdown
												options={faculties}
												selected={faculties.find(
													(ff: any) => ff.id === facultyId
												)}
										...
										<View
											className="w-11/12 mb-3 rounded-3xl"
											style={[
												styles.inputShadow,
												{
													backgroundColor: "#fff",
										...
										{studyEnum.length === 0 ? (
											...
											<Dropdown
												options={studyEnum}
												selected={studyEnum.find(
													(x: any) => x.enumlabel === studyStatus
												)}
										...
										{clubEnum.length === 0 ? (
											...
											<Dropdown
												options={clubEnum}
												selected={clubEnum.find(
													(x: any) => x.enumlabel === clubStatus
												)}
